...
import {
  ...
  MapPin, Route, Send, Plus, X, Search, Loader2,
...
interface Stop {
  ...
  items:             ManifestItem[];   // only populated for pickup stops
}
...
function StopCard({
  ...
  function set<K extends keyof Stop>(k: K, v: Stop[K]) {
    onUpdate({ ...stop, [k]: v });
  }
        ...
        {isPickup && (
          <div className="mt-2 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={addItem}
                disabled={dropoffStops.length === 0}
              ...
              <div key={item.localId} className="rounded-lg border border-[var(--color-cream-dark)] bg-[var(--color-cream)] p-3 space-y-2">
                <div className="flex items-start gap-2">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      placeholder="e.g. Widgets, Spare Parts"
                      className={inputCls}
                    />
                  </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        value={item.deliveryStopLocalId}
                        onChange={e => updateItem(idx, { deliveryStopLocalId: e.target.value })}
                        className={`${inputCls} appearance-none pr-6`}
                      >
                        {dropoffStops.map(d => (
                          <option key={d.localId} value={d.localId}>
                            {d.city || `Dropoff ${d.sequence}`}
                          </option>
                        ))}
                      </select>
                      <ChevronDown size={12} className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-[var(--color-text-faint)]" />
          ...
          <div key={stop.id} className={`flex items-center gap-4 px-4 py-3 ${i < ordered.length - 1 ? 'border-b border-[var(--color-cream-dark)]' : ''}`}>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-[var(--color-text)] capitalize">
                {stop.stop_type} — {stop.city}, {stop.state}
              </p>
            ...
            {stop.estimated_arrival_at && (
              <p className="shrink-0 text-xs text-[var(--color-teal)] font-medium">
                est. {new Date(stop.estimated_arrival_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
              </p>
            )}
          </div>
        ))}
...
export default function NewContractedJobPage() {
  ...
  function addStop(type: 'pickup' | 'dropoff') {
    ...
    setStops(prev => [...prev, {
      localId: uid(), stopType: type, sequence: seq,
      contactName: '', contactPhone: '',
      address: '', city: '', state: '', zip: '',
      scheduledDate: '', windowStart: '', windowEnd: '',
      weightLbs: '', specialInstructions: '',
      items: [],
    }]);
  }
  ...
  function removeStop(localId: string) {
    setStops(prev => {
      ...
      // Remove manifest items referencing the removed stop
      return next.map(s => ({
        ...s,
        items: s.items.filter(it => it.deliveryStopLocalId !== localId),
      }));
    });
  }
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {step === 0 && (
        <div className="space-y-5">
          <div className="rounded-2xl border border-[var(--color-cream-dark)] bg-[var(--color-white)] p-6 space-y-4">
            ...
            {contractId && (
              <div>
                <label className={labelCls}>Route optimization</label>
                <div className="grid grid-cols-2 gap-2">
                  {([
                    ...
                    <button
                      key={opt.value}
                      onClick={() => setOptimMode(opt.value)}
      ...
      {step === 1 && (
        <div className="space-y-5">
          ...
          <div className="rounded-2xl border border-[var(--color-cream-dark)] bg-[var(--color-white)] p-5 space-y-4">
            ...
            <div>
              ...
              <textarea value={instructions} onChange={e => setInstructions(e.target.value)}
                rows={2} placeholder="Fragile cargo, call ahead at each stop…"
          ...
          <div className="space-y-3">
            ...
            {pickupStops.map((stop, i) => (
              <StopCard
                ...
                index={i}
                onUpdate={s => updateStop(stop.localId, s)}
                onRemove={() => removeStop(stop.localId)}
              />
            ))}
          </div>
          ...
          <div className="flex items-center justify-between pt-2">
            <button onClick={() => setStep(0)} className="flex items-center gap-1.5 text-sm font-semibold text-[var(--color-text-muted)] hover:text-[var(--color-text)] transition-colors">
      ...
      {step === 2 && (
        <div className="space-y-5">
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-[var(--color-text-faint)] mt-0.5">
                  System will calculate the shortest route respecting all pickup-before-delivery constraints
                </p>
              </div>
          ...
          <div className="flex items-center justify-between">
            <button
              onClick={() => { setStep(1); setOptimised(false); }}
      ...
      {step === 3 && createdJob && (
        <div className="space-y-5">
          ...
          <div className="flex items-center justify-between">
            <button onClick={() => setStep(2)} className="flex items-center gap-1.5 text-sm font-semibold text-[var(--color-text-muted)] hover:text-[var(--color-text)] transition-colors">
              <ChevronLeft size={15} /> Back
            </button>
            <button
