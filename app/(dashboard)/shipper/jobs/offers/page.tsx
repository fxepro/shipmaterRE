...
export default function ShipperOffersPage() {
  ...
  const acceptMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Bid accepted — job is now assigned.');
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-2xl text-[var(--color-slate)]" style={{ fontFamily: 'var(--font-display)' }}>Incoming Offers</h1>
          ...
          {Array.from({ length: 2 }).map((_, i) => (
            <div key={i} className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {Array.from({ length: 3 }).map((_, j) => <div key={j} className="h-44 rounded-xl bg-[var(--color-cream)] animate-pulse" />)}
              </div>
            </div>
          ))}
        </div>
      ) : shipments.length === 0 ? (
        <EmptyState icon={Radio} title="No incoming bids" description="When carriers bid on your open jobs, they'll appear here for review." />
      ) : (
        <div className="space-y-8">
                ...
                <div className="mb-4 flex items-center gap-3">
                  <div>
                    ...
                    <div className="mt-0.5 flex items-center gap-1.5 text-xs text-[var(--color-text-muted)]">
                      ...
                      {shipment.pickup_date && <span className="text-[var(--color-text-faint)]">· Pickup {formatDate(shipment.pickup_date)}</span>}
                    </div>
                  </div>
